...
public class TensesActivity extends AppCompatActivity {
    ...
    private TensesRecyclerViewAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new TensesRecyclerViewAdapter(this, tensesModels);
    ...
    private void loadInitialProgressData() {
        ...
        DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("users").child(userId).child("tenses");

        userRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (int i = 0; i < tensesModels.size(); i++) {
                    TensesModel model = tensesModels.get(i);
                    String tenseKey = model.getTenseName().replace(" ", "");

                    if (snapshot.hasChild(tenseKey)) {
                        Integer progress = snapshot.child(tenseKey).getValue(Integer.class);
                        if (progress != null) {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(TensesActivity.this, "Failed to load progress", Toast.LENGTH_SHORT).show();
            }
        });
    }
